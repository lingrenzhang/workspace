...
<%@ page import="com.hitchride.global.Environment"%>
<%
...
<%
	...
	TransientRide tranRide = (TransientRide) request.getSession()
			.getAttribute("tranRide");
%>

<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">

<link rel="stylesheet"
	href="/TicketSchedule/bootstrap/css/bootstrap.css">
<link href="/TicketSchedule/CSS/master.css" type="text/css"
	rel="stylesheet">
<link href="/TicketSchedule/CSS/searchtransientride.css" type="text/css"
	rel="stylesheet">
<link href="/TicketSchedule/CSS/custom_jqueryui.css" type="text/css"
	rel="stylesheet">
...
-->
<script type="text/javascript"
	src="http://api.map.baidu.com/api?v=1.5&ak=Mto5Y3Pq2fgwkY2Kt9n60bWl"></script>
<script type="text/javascript">
...
$(document).ready(function(){
	...
	origAddr= "<%=(tranRide == null) ? "" : tranRide.origLoc
					.get_formatedAddr()%>";
	destLat="<%=tranRide == null ? "" : tranRide.destLoc.get_lat()%>";
	destLng="<%=tranRide == null ? "" : tranRide.destLoc.get_lon()%>";
	destAddr= "<%=(tranRide == null) ? "" : tranRide.destLoc
					.get_formatedAddr()%>
	";
						document.getElementById("search_s").setAttribute(
								"value", origAddr);
						document.getElementById("search_e").setAttribute(
								"value", destAddr);

						if (origAddr != "") {
							document.getElementById("search_s").setAttribute(
									"placeholder", origAddr);
						}
						if (destAddr != "") {
							document.getElementById("search_e").setAttribute(
								...
								.addEventListener(
										...
										function(e) {
											...
											function myFun() {
												...
												map.addOverlay(omarker);
												document
														.getElementById("origLat").value = point.lat;
												document
														.getElementById("origLng").value = point.lng;
												origLat = point.lat;
												origLng = point.lng;
												destLat = document
														.getElementById("destLat").value;
												destLng = document
														.getElementById("destLng").value;
												if (destLat != ""
														&& destLng != "") {
													var oLatlng = new BMap.Point(
															origLng, origLat);
													var dLatlng = new BMap.Point(
															destLng, destLat);
													basicbounds = new BMap.Bounds(
															oLatlng, dLatlng);
													refitb(basicbounds);
								...
								.addEventListener(
										...
										function(e) {
											...
											function myFun() {
												...
												map.addOverlay(dmarker);
												document
														.getElementById("destLat").value = point.lat;
												document
														.getElementById("destLng").value = point.lng;
												destLat = point.lat;
												destLng = point.lng;
												origLat = document
														.getElementById("origLat").value;
												origLng = document
														.getElementById("origLng").value;
												if (origLat != ""
														&& origLng != "") {
													var oLatlng = new BMap.Point(
															origLng, origLat);
						...
						function calculateDistances() {
							...
							document.getElementById("distance").setAttribute(
									"value", distance);
							document.getElementById("duration").setAttribute(
									"value", duration);

							var price = Math.floor(distance / 1200);
							document.getElementById("price").setAttribute(
									"value", price);
						}
	...
	function search() {
		...
		queryURL = queryURL + "?s=" + origAddr + "&origLat=" + origLat
				+ "&origLng=" + origLng;
		queryURL = queryURL + "&e=" + destAddr + "&destLat=" + destLat
				+ "&destLng=" + destLng;
		queryURL = queryURL + "&distance=" + distance + "&duration" + duration;
		queryURL = queryURL + "&date=" + date;
		document.getElementById("headline").innerHTML = "<span>出发日：" + date
				+ "</span>";
		...
		$(".entry").hover(
				function() {
					...
					tomarker = new BMap.Marker(toLatlng, {
						icon : images
					});
					tdmarker = new BMap.Marker(tdLatlng, {
						icon : imagee
					});

					var bounds = new BMap.Bounds(basicbounds.getSouthWest(),
							basicbounds.getNorthEast());
...
<script>
	...
	function getTransientRide(trInfo, rank) {
		var topicstring = "";
		topicstring = topicstring + "<a href=\"./TransientTopic.jsp?trId="
				+ trInfo.transientRideId + "\">";
		topicstring = topicstring
				+ "<div class=\"entry\" origLat="+trInfo.origLoc._lat+" ";
		...
		if (trInfo.userType) {
			...
			topicstring = topicstring
					+ "<span><img src='/TicketSchedule/Picture/nocar.jpg'/><br><span>";
			topicstring = topicstring + trInfo.owner._givenname
					+ "<br> <strong>不提供车</strong></p></div>";
		} else {
			topicstring = topicstring
					+ "<div class=\"price_box\"><div class=\"seats\">";
			topicstring = topicstring
					+ "<img src='/TicketSchedule/Picture/seats.jpg'/><span class='count'>"
					+ trInfo.totalSeats + "</span></div>";
			topicstring = topicstring + "<p><b>" + trInfo.price
					+ "</b> / 座</p></div>";
		}

		topicstring = topicstring + "<div class=\"userpic\">";
		topicstring = topicstring + "<div class=\"username\">"
				+ trInfo.owner._givenname + "</div>";
		topicstring = topicstring
				+ "<img src= \"/TicketSchedule/UserProfile/"+trInfo.owner._avatarID+"\" alt=\"Profile Picture\"></img>";
		...
		topicstring = topicstring
				+ "<span class=\"inner\"> <img src=\"/TicketSchedule/Picture/pin_start.png\"/>"
				+ "  出发地：" + trInfo.origLoc._addr + "<br>";
		topicstring = topicstring
				+ "<span class=\"inner\"> <img src=\"/TicketSchedule/Picture/pin_end.png\"/>"
				+ "  目的地：" + trInfo.destLoc._addr + "<br>";
		topicstring = topicstring
				+ "<span class=\"inner\"> <img src=\"/TicketSchedule/Picture/clock_small.jpg\"/>"
				+ " 出发时间：" + trInfo.rideTime + "<br>";
...
<script type="text/javascript">
	function publishRide() {
		...
		if (onPublishValidate()) {
			...
			{
				queryURL = queryURL + "?s="
						+ document.getElementById("search_s").value;
			} else {
			...
			{
				queryURL = queryURL + "&e="
						+ document.getElementById("search_e").value;
			} else {
			...
			if (document.getElementById("ride_time_ap").value == "AM") {
				...
				time_hour = Number(document.getElementById("ride_time_hour").value)
						+ Number(12);
			}
			...
			queryURL = queryURL + "&price="
					+ document.getElementById("price").value;
			queryURL = queryURL + "&seats="
					+ document.getElementById("seats").value;

			var tranid = getJson(queryURL);
			window.location.href = "/TicketSchedule/Zh/TransientTopic.jsp?trId="
					+ tranid;
		} else {
			return false;
		}
	...
	function onPublishValidate() {
		if (document.getElementById("origLat").value == ""
			...
			document.getElementById("additional-info").setAttribute("class",
					"panel");
			return false;
		}
...
<body id="search_index">
				...
				<div id="head">
					<div class="search">
						<div class="text_input">
							<label class="pin start" for="search_s"></label> <input
								id="search_s" class="input_text" type="text"
								placeholder="Starting from..." name="s" alt="search_start"
								value="" />
						</div>
						<div class="text_input">
							<label class="pin end" for="search_e"></label> <input
								id="search_e" class="input_text" type="text"
								placeholder="Going to..." name="e" alt="search_end" value="" />
						</div>
						<div class="geo_internal" style="display: none">
							<input id="origLat" name="origLat" value=""></input> <input
								id="origLng" name="origLng" value=""></input> <input
								id="destLat" name="destLat" value=""></input> <input
								id="destLng" name="destLng" value=""></input> <input
								id="distance" name="distance"
								value="<%=tranRide == null ? "" : tranRide.dist%>"></input> <input
								id="duration" name="duration"
								value="<%=tranRide == null ? "" : tranRide.dura%>"></input>
						</div>
						<div class="text_input datetime">
							<label class="datetime_icon" for="search_date"></label> <input
								id="search_date" class="slim datepicker hasDatepicker"
								type="text" value="exp" name="date">
						</div>
					...
					<div class="ride_list">
						...
						<div id="action">
							<div class="item postride">
								<h2>
									<a href="">没有找到你要的临时拼车信息? </a>
								</h2>
								<p>补充更多临时拼车信息并发布，其他人可以找到并加入你!</p>
								<button id="createTopic" type="submit" class="button post"
									onclick="publishRide()">发布临时拼车</button>
							</div>
						</div>
					</div>
				</div>
								...
								<div class="panel-body">
									<div class="tabbable tabs-top">
										<ul class="nav nav-tabs">
											<li class="" id="asDriver"><a
												href="javascript: asDriver()"><img
													src="/TicketSchedule/Picture/car.jpg"></img>有车</a></li>
											<li class="active" id="asPassenger"><a
												href="javascript: asPassenger()"><img
													src="/TicketSchedule/Picture/nocar.jpg"></img>无车</a></li>
										</ul>
									</div>
									<div id="bargin-content">
										<div id="seats-content" style="display: none">
											<img src="/TicketSchedule/Picture/seats.jpg"></img> <input
												type="text" id="seats" value="3" />
										</div>
										<div id="price-content">
											<img src="/TicketSchedule/Picture/yuansign.jpg"></img> <input
												type="text" id="price" value="15" />
										</div>
									</div>
									<div id="schedule-info">
										<img src="/TicketSchedule/Picture/clock.jpg" /> <select
											name="ride_time_ap" id="ride_time_ap">
											<option value="AM">上午</option>
											<option value="PM">下午</option>
										</select> <select name="ride_time_hour" id="ride_time_hour"
											class="slim">
										...
										</select>点 <select name="ride_time_minute" id="ride_time_minute"
											class="slim">
									...
									<div>
										<button id="publishTopic" type="submit" class="button post"
											onclick="publishRide()">发布</button>
									</div>
								</div>
							</div>
						</div>
	...
	<div id="ui-datepicker-div"
		...
		<div
			...
			<a class="ui-datepicker-prev ui-corner-all" onclick="prevMonth()"
				title="Prev"> <span class="ui-icon ui-icon-circle-triangle-e">Prev</span>
			</a> <a class="ui-datepicker-next ui-corner-all" onclick="nextMonth()"
				title="Next"> <span class="ui-icon ui-icon-circle-triangle-w">Next</span>
			</a>
			<div class="ui-datepicker-title">
				<span class="ui-datepicker-month" id="picker-Month"></span> &nbsp; <span
					class="ui-datepicker-year" id="picker-Year"></span>
			</div>
		</div>
